...
public class Main extends JPanel {
    ...
    public void writeInFile(long timeXt, double mathExpectation, double dispersion, long timeRxx, long timeRxy) throws Exception {
        FileWriter nFile = new FileWriter("C:\\Users\\aaast\\RTS_reports\\file1.txt");
    ...
    public void paintComponent(Graphics g) {
        ...
        double mathExpectation = findMathExpectation(x, N);
        double[] Rxx = findRxx(N, x, mathExpectation);
        g.setColor(new Color(5));
        try {
            for (int i = 0; i < (N/2) - 1; i++) {
                if (i != (N/2) - 1) {
                    g3.draw(new Line2D.Double(i*10, 500 + 6 * Rxx[i], (i + 1)*10, 500 + 6 * Rxx[i + 1]));
                }
            }
        } catch (ArrayIndexOutOfBoundsException e) {}
        y = generateRandSignal(N, n, aY, fiY, freq);
        double mathExpectationY = findMathExpectation(y, N);
        double[] Rxy = findRxy(N, y, mathExpectation, mathExpectationY);
        g.setColor(new Color(255));
        try {
            for (int i = 0; i < (N/2) - 1; i++) {
                if (i != (N/2) - 1) {
                    g4.draw(new Line2D.Double(i*10, 500 + 6 * Rxy[i], (i + 1)*10, 500 + 6 * Rxy[i + 1]));
                }
            }
        } catch (ArrayIndexOutOfBoundsException e) {}
    }
